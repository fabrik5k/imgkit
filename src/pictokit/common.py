...
def validate_imgarray(img_arr: np.ndarray, mode: Mode = "any") -> np.ndarray:
    ...
    if img_arr.ndim == GREY_SCALE_DIM:
        if mode in ("gray", "any"):
            return img_arr
        raise ValueError(f"Expected color image, got grayscale with shape {img_arr.shape}.")

    if img_arr.ndim == RGB_DIM and img_arr.shape[2] == RGB_CHANNELS:
        if mode in ("color", "any"):
            return img_arr
        raise ValueError(f"Expected grayscale image, got color with shape {img_arr.shape}.")
